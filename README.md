...
|-------|---------|-------------|
| **flow** | `npx skills add kzarzycki/agentic-workflow@flow -g -y` | Persistent workflow orchestration. Survives context resets, delegates to subagents, tracks work items. |
...
```bash
npx skills add kzarzycki/agentic-workflow@<skill> -g -y
```
...
SKILL=flow && \
  ...
  cp -r /tmp/agentic-workflow-dl/$SKILL/* .claude/skills/$SKILL/ && \
