...
`vendir.yml` is reviewed acquisition policy. Carvel vendir resolves and copies
the declared Git content. For each source the configuration declares:
...
`vendir.lock.yml` is generated by vendir and records the resolved full Git
commit. Locked reproduction uses that file instead of resolving a moving
branch.
...
1. Validates `vendir.yml` and the package policy.
2. Runs vendir in a temporary package tree, using `vendir.lock.yml` for locked
   reproduction or resolving the tracked ref for an intentional refresh.
