...
One or more vendir manifests define reviewed acquisition policy. Carvel vendir
resolves and copies the declared Git content. The primary `vendir.yml` manages
the imported engineering collection. `vendir.grilling.yml` manages
`skills/grilling`; the primary manifest preserves that path along with
repository-owned skills. Each manifest declares:
...
- repository-owned paths preserved during synchronization.

Each manifest has a generated lock file recording the resolved full Git commit.
Locked reproduction uses those files instead of resolving a moving branch.
All Matt source locks must resolve to the same commit for a package release.
...
1. Validates every vendir manifest, lock, and the package policy.
2. Runs each vendir manifest in a temporary package tree, using its lock for
   locked reproduction or resolving the tracked ref for an intentional
   refresh.
3. Discovers paths matching vendir's include and exclude policy.
4. Compares the discovered skill inventory with the previous provenance.
5. Stops for upstream removals or renames.
6. Applies the ordered patch set to the temporary tree.
7. Adds repository-owned skills and metadata.
8. Copies required licenses and notices.
9. Generates provenance and content hashes.
10. Runs package tests against the temporary tree.
11. Compares the result with the committed package.
12. Replaces generated paths only after successful validation.

Fetch, patch, license, or test failure leaves the committed capability pack unchanged.

Vendir owns Git resolution, content filtering, copying, and the primary source
lock. The qualification command owns integration patches, inventory policy,
extended provenance, owned-path protection, staged validation, and update
evidence. It does not implement another Git fetch or copy engine.
