...
**R14 — Explicit local behavior.** Repository-owned skills live beside imported skills and have distinct names. Vendir owns only the declared imported skill leaves, so synchronization preserves owned sibling directories. Replacement or adaptation skills do not shadow an upstream skill name.
...
**R18 — Engineering inventory.** The first `engineering` release imports every current `skills/engineering/*/SKILL.md` capability from `mattpocock/skills`, maps the upstream setup leaf to `setup-engineering-workflow-for-apm`, and adds `skills/productivity/grilling`.

**R19 — APM setup skill.** A repository-owned patch transforms the imported setup payload into `setup-engineering-workflow-for-apm`, which writes project-owned instruction sources under `.apm/instructions/` and repository guidance under `docs/agents/`.
...
The package's vendir manifest defines reviewed acquisition policy. Carvel
vendir resolves and copies the declared Git content. `vendir.yml` contains one
managed directory per imported skill leaf. Each entry maps one upstream leaf
to its final sibling under `engineering/skills/`; the setup entry maps
`setup-matt-pocock-skills` directly to
`setup-engineering-workflow-for-apm`. `grilling` is another leaf entry.
Repository-owned sibling directories fall outside every managed destination.
...
- include rules;
- a final destination for each imported leaf;
- legal files copied with the payload;
- the complete imported inventory.

The generated lock records the resolved full Git commit for every leaf.
Locked reproduction uses it instead of resolving a moving branch. All entries
from the Matt source must resolve to the same commit for a package release.
During intentional refresh, inventory reconciliation enumerates the immediate
directories under the configured upstream engineering collection at the
candidate commit. It adds deterministic leaf entries for new skills and stops
when a configured upstream leaf disappeared or changed name.

`provenance.yml` is generated by the repository qualification command. It
records the included and excluded skill inventory, source-to-destination path
mapping, source hashes, applied patch hashes, license hashes, and generated
payload hashes. The ordered patch series remains under `patches/`.
...
The vendoring operation composes vendir with one repository-owned qualification
command:

1. Validates the vendir manifest, lock, and package policy.
2. For an intentional refresh, resolves the candidate commit in a temporary
   shallow, filtered Git checkout and reconciles the manifest's leaf inventory.
   The staged vendir entries are pinned to that candidate for acquisition, then
   retain the configured tracked ref in the committed manifest.
3. Runs vendir in a temporary package tree, using its lock for
   locked reproduction or resolving the tracked ref for an intentional
   refresh.
4. Discovers paths matching vendir's declared leaf policy.
5. Compares the discovered skill inventory with the previous provenance.
6. Stops for upstream removals or renames.
7. Applies the ordered patch set to the temporary tree.
8. Verifies repository-owned sibling skills and metadata are unchanged.
...
Vendir owns payload acquisition, leaf-level content filtering, copying, and
the source lock. Refresh-time inventory reconciliation uses Git only to list
upstream skill directories at the candidate commit; it does not copy payload
files into the package. The qualification command owns integration patches,
inventory policy, extended provenance, owned-sibling protection, staged
validation, and update evidence.
...
Imported files are verified by reconstruction. Owned skill directories are
sibling paths outside vendir's declared leaf destinations and follow normal
repository review.
...
The patched `setup-engineering-workflow-for-apm` skill:
...
The recorded APM integration patch changes the imported setup payload, its
directory name, and references from other imported skills. The resulting
installed skill catalog contains `setup-engineering-workflow-for-apm`.
...
Renovate is the preferred weekly update trigger because it supports vendir lock
maintenance and moving Git refs. A fixture must prove that the configured
Renovate path commits the regenerated payload together with
`vendir.lock.yml`. If it cannot, a short scheduled GitHub workflow runs vendir
and the same qualification command. Both paths open draft pull requests with
automerge disabled.
...
**AC8 — Complete Matt inventory.** The generated engineering pack contains every current upstream `skills/engineering/*` capability, with setup mapped to `setup-engineering-workflow-for-apm`, plus `grilling`.
