...
| Spawn | `sys_session_create(agent_id, message?, title?)` | runs **async**, returns `conversation_id`. **Children only** — no top-level/sibling sessions. **No completion signal:** a bare `create` delivers NOTHING to your inbox and fires no wake — you MUST poll `get_info` yourself (or drive it with `sys_session_send`, which *does* notify). `config_path=` instead of `agent_id` uploads + launches a fresh agent from a local YAML/dir/.tar.gz. |
| Drive / delegate more | `sys_session_send(session_id, args)` | posts a turn to a child. **Dispatches async** — returns a `launching` handle, the result lands in the inbox (a completion notification fires); poll `get_info` or `sys_read_inbox`, don't expect an inline return. Parallel fan-out: emit several calls in one response. Sending to an already-**running** child queues an override that supersedes at the next turn boundary — this is the lever for redirecting a child mid-run (see Gotchas). |
...
Minimal loop: `create(agent_id, message=task)` → **poll `get_info`** until `status` is `idle` or `failed` → `get_history(tail_items=N)` for the result. A bare `create` returns nothing inline AND sends nothing to your inbox / fires no wake — so if you background it, **set a `ScheduleWakeup` to come back and poll**, or you'll never hear back. `sys_session_send`/`sys_call_async` are the *notifying* path (completions land in the inbox); `create` alone is silent.
