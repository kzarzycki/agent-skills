...
Creates a new repository from the Copier template in `template/`, applying the
...
2. **Infer optionals from context — don't interrogate.** `description`,
   `include_docker`, `include_release_please` default sensibly (see
   `template/copier.yml` for the full question set and defaults). Pass an
...
- To update a generated project later when the template improves:
  `cd <project> && copier update`.
- Full template internals and the per-language file inventory live under
  `template/` — read `template/copier.yml` to see every question and default.
